...
import pl.project.clusteringspringboot.spatialindex.storagemanager.*;
...
public class RTree implements ISpatialIndex
{
    ...
    /**
     * 
     * @param size    size of the datset
     * @param eps     radius of the search
     * @param query   
     * @param v
     * @param nnc
     */
    public void nearestNeighborQuery(double eps, final IShape query,
        final IVisitor v, int size) {
        ...
        try {
            ...
            // unique keys only and since I am sorting according to distances,
            // it is not assured that all distances will be unique. TreeMap
            ...
            while (queue.size() != 0) {
                ...
                if (first.m_pEntry instanceof Node) {
                    ...
                    // distances. (neighbors can be more than k, if many happen
                    // to have the same furthest distance).
                    if (first.m_minDist > eps)
                        break;
